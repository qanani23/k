...
fn test_claim_id_logged_when_constructing_url() {
    ...
    assert!(result.is_ok(), "parse_claim_item should succeed for valid stream claim");
...
fn test_constructed_url_logged() {
    ...
    assert!(url.contains("url-test-claim"), "URL should contain claim_id");
...
fn test_ambiguous_claim_structure_logged() {
    ...
    assert!(result.is_ok(), "Should succeed for ambiguous claim with source.sd_hash");
...
fn test_empty_claim_id_logged_with_context() {
    ...
    let error_msg = error.to_string();
    assert!(
        error_msg.contains("claim_id") || error_msg.contains("empty") || error_msg.contains("Missing"),
...
fn test_gateway_logged_at_startup() {
    ...
    assert!(!gateway.ends_with('/'), "Gateway should not have trailing slash");
...
fn test_cdn_url_construction_deterministic() {
    ...
    assert!(url1.contains(claim_id), "URL should contain claim_id for logging traceability");
}
