...
pub fn sanitize_order_by(order_by: &str) -> Result<String> {
    ...
    for part in parts {
        ...
        if !ALLOWED_COLUMNS.contains(&column) {
            ...
            return Err(KiyyaError::InvalidInput {
                message: format!(
                    "Invalid ORDER BY column: '{}'. Allowed columns: {:?}",
        ...
        let direction = if tokens.len() > 1 {
            ...
            if !ALLOWED_DIRECTIONS.contains(&dir.as_str()) {
                return Err(KiyyaError::InvalidInput {
                    message: format!(
                        "Invalid ORDER BY direction: '{}'. Allowed: ASC, DESC",
                        tokens[1]
                    ),
                });
            }
...
mod tests {
    ...
    fn test_sanitize_order_by_valid() {
        ...
        assert_eq!(
            sanitize_order_by("releaseTime ASC").unwrap(),
            "releaseTime ASC"
        );

        // Single column with DESC
        assert_eq!(
            sanitize_order_by("releaseTime DESC").unwrap(),
            "releaseTime DESC"
        );
    ...
    fn test_sanitize_like_pattern() {
        ...
        assert_eq!(
            sanitize_like_pattern("test%pattern").unwrap(),
            "test\\%pattern"
        );
        assert_eq!(
            sanitize_like_pattern("test_pattern").unwrap(),
            "test\\_pattern"
        );
        assert_eq!(
            sanitize_like_pattern("test[pattern").unwrap(),
    ...
    fn test_sanitize_fts5_query() {
        ...
        assert_eq!(
            sanitize_fts5_query("test \"quoted\" text").unwrap(),
            "\"test \"\"quoted\"\" text\""
        );
        ...
        // Complex query
        assert_eq!(
            sanitize_fts5_query("season 1 episode 5").unwrap(),
            "\"season 1 episode 5\""
        );
    }
}
