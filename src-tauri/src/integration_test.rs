...
mod tests {
    ...
    use crate::models::{ContentItem, FavoriteItem, Playlist, PlaylistItem, ProgressData};
    use chrono::Utc;
    ...
    async fn test_fresh_database_initialization() {
        ...
        let cached_content = db
            .get_cached_content(crate::models::CacheQuery {
                tags: None,
                text_search: None,
                order_by: None,
                limit: Some(10),
        ...
        assert_eq!(
            cached_content.len(),
            0,
            "Should start with no cached content"
        );
    ...
    async fn test_existing_database_compatibility() {
        ...
        apply_migrations_up_to(&db_path, 10).expect("Failed to apply migrations 1-10");
        ...
        let version_before = get_current_version(&db_path).expect("Failed to get version");
        ...
        let version_after = get_current_version(&db_path).expect("Failed to get version after");
        assert_eq!(
            version_after, 14,
            "Should be at version 14 after applying remaining migrations"
        );
    ...
    async fn test_fts5_initialization() {
        ...
        if fts5_available {
            ...
            let test_content = vec![ContentItem {
            ...
            db.store_content_items(test_content)
                .await
                .expect("Failed to store content");

            // Test FTS5 search
            let search_results = db
                .search_content("matrix", Some(10))
                .await
    ...
    async fn test_error_logging() {
        ...
        assert!(
            result.is_ok(),
            "Getting nonexistent playlist should not error"
        );
        assert!(
        ...
        assert!(
            result.is_ok(),
            "Deleting nonexistent progress should not error"
        );
        ...
        assert!(
            favorites.is_ok(),
            "Database should still work after error operations"
        );
    ...
    async fn test_multiple_startup_cycles() {
        ...
        for i in 0..5 {
            ...
            let migration_count =
                get_migration_count(&db_path).expect("Failed to get migration count");
            assert_eq!(
                migration_count, 14,
    ...
    async fn test_concurrent_database_access() {
        ...
        assert_eq!(
            favorites.len(),
            10,
            "Should have 10 favorites from concurrent operations"
        );
    ...
    async fn test_hero_section_with_valid_hero_trailer() {
        ...
        video_urls.insert(
            ...
            crate::models::VideoUrl {
                url: "https://cloud.odysee.live/content/test_hero_claim_123/master.m3u8"
                    .to_string(),
                quality: "master".to_string(),
        ...
        assert_eq!(
            hero.claim_id, "test_hero_claim_123",
    ...
    async fn test_hero_section_with_missing_direct_urls() {
        ...
        video_urls.insert(
            "master".to_string(),
            crate::models::VideoUrl {
                url: "https://cloud.odysee.live/content/hero_no_direct_urls/master.m3u8"
                    .to_string(),
                quality: "master".to_string(),
                url_type: "hls".to_string(),
                codec: None,
        ...
        let hero_item = ContentItem {
            ...
            description: Some(
                "This hero item has no direct URL fields in API response".to_string(),
            ),
        ...
        assert_eq!(
            hero.claim_id, "hero_no_direct_urls",
    ...
    async fn test_series_section_with_partial_success() {
        ...
        assert_eq!(
            cached_items[0].claim_id, "series_ep1_claim",
            "Most recent episode should be first"
        );
        assert_eq!(
            cached_items[1].claim_id, "series_ep2_claim",
            "Second most recent episode should be second"
        );
        assert_eq!(
            cached_items[2].claim_id, "series_ep3_claim",
            "Oldest episode should be last"
        );
        ...
        let limited_items = db
            .get_cached_content(limited_query)
            .await
        ...
        let offset_items = db
            .get_cached_content(offset_query)
            .await
    ...
    async fn test_movies_section_with_all_claims_missing_claim_id() {
        ...
        let favorites = db
            .get_favorites()
            .await
            .expect("Database should still be operational");
        ...
        let cached_items_after = db
            .get_cached_content(query_after)
            .await
        ...
        assert_eq!(
            cached_items_after[0].claim_id, "valid_movie_claim",
        ...
        let series_items = db
            .get_cached_content(series_query)
            .await
            .expect("Failed to query series content");
        ...
        let final_movies = db
            .get_cached_content(final_movie_query)
            .await
        ...
        assert_eq!(
            final_movies[0].claim_id, "valid_movie_claim",
        ...
        let final_series = db
            .get_cached_content(final_series_query)
            .await
        ...
        assert_eq!(
            final_series[0].claim_id, "series_claim",
    ...
    async fn test_section_independence_hero_failure_doesnt_affect_series() {
        ...
        for i in 1..=3 {
            ...
            video_urls.insert(
                ...
                crate::models::VideoUrl {
                    url: format!(
                        "https://cloud.odysee.live/content/series_ep{}_claim/master.m3u8",
                        i
                    ),
                    quality: "master".to_string(),
                    url_type: "hls".to_string(),
        ...
        for i in 1..=2 {
            ...
            video_urls.insert(
                ...
                crate::models::VideoUrl {
                    url: format!(
                        "https://cloud.odysee.live/content/movie{}_claim/master.m3u8",
                        i
                    ),
                    quality: "master".to_string(),
                    url_type: "hls".to_string(),
        ...
        let movies_result = db
            .get_cached_content(movies_query)
            .await
        ...
        let empty_series_result = db
            .get_cached_content(empty_series_query)
            .await
        ...
        let movies_result_after = db
            .get_cached_content(movies_query_after)
            .await
        ...
        let hero_final = db
            ...
            .await
            .expect("Hero query should succeed");
        ...
        let movies_final = db
            ...
            .await
            .expect("Movies query should succeed");
        ...
        let hero_recovered = db
            ...
            .await
            .expect("Hero query should succeed after recovery");
        ...
        let series_still_ok = db
            ...
            .await
            .expect("Series query should still succeed");
        ...
        let movies_still_ok = db
            ...
            .await
            .expect("Movies query should still succeed");
    ...
    async fn test_tag_based_content_discovery_still_works() {
        ...
        assert_eq!(hero_results.len(), 1, "Should find 1 hero_trailer item");
        ...
        let movie_results = db
            .get_cached_content(movie_query)
            .await
            .expect("Failed to query movie content");

        assert_eq!(movie_results.len(), 1, "Should find 1 movie item");
        ...
        let series_results = db
            .get_cached_content(series_query)
            .await
            .expect("Failed to query series content");

        assert_eq!(series_results.len(), 1, "Should find 1 series item");
        ...
        let sitcom_results = db
            .get_cached_content(sitcom_query)
            .await
            .expect("Failed to query sitcom content");

        assert_eq!(sitcom_results.len(), 1, "Should find 1 sitcom item");
        ...
        let kids_results = db
            .get_cached_content(kids_query)
            .await
            .expect("Failed to query kids content");

        assert_eq!(kids_results.len(), 1, "Should find 1 kids item");
        ...
        let action_movies_results = db
            .get_cached_content(action_movies_query)
            .await
        ...
        assert!(action_movies_results[0]
            .tags
            .contains(&"action_movies".to_string()));
        assert!(
        ...
        let comedy_series_results = db
            .get_cached_content(comedy_series_query)
            .await
        ...
        assert!(comedy_series_results[0]
            .tags
            .contains(&"comedy_series".to_string()));
        assert!(
        ...
        let action_kids_results = db
            .get_cached_content(action_kids_query)
            .await
        ...
        assert!(action_kids_results[0]
            .tags
            .contains(&"action_kids".to_string()));
        assert!(
        ...
        let multi_tag_results = db
            .get_cached_content(multi_tag_query)
            .await
        ...
        let claim_ids: Vec<String> = multi_tag_results
            .iter()
        ...
        let nonexistent_results = db
            .get_cached_content(nonexistent_query)
            .await
        ...
        assert_eq!(
            all_results[0].claim_id, "hero_claim",
        ...
        assert_eq!(
            all_results[3].claim_id, "sitcom_claim",
            "Fourth most recent"
        );
        assert_eq!(
            all_results[4].claim_id, "kids_claim",
