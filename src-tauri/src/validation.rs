...
pub fn validate_claim_id(claim_id: &str) -> Result<String> {
    ...
    if !claim_id.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '@' || c == ':' || c == '#' || c == '/') {
        log_security_event(SecurityEvent::InputValidationFailure {
            input_type: "claim_id".to_string(),
...
pub fn validate_quality(quality: &str) -> Result<String> {
    ...
    if !VALID_QUALITIES.contains(&normalized.as_str()) {
        return Err(KiyyaError::InvalidInput {
            message: format!("Invalid quality: '{}'. Must be one of: {:?}", quality, VALID_QUALITIES),
        });
    }
...
pub fn validate_external_url(url: &str) -> Result<String> {
    ...
    let host = parsed_url.host_str().ok_or_else(|| KiyyaError::InvalidInput {
    ...
    let is_approved = APPROVED_DOMAINS.iter().any(|domain| {
...
///
/// Search queries should not be empty and have reasonable length
...
pub fn validate_position_seconds(position: u32) -> Result<u32> {
    ...
    if position > MAX_POSITION {
        return Err(KiyyaError::InvalidInput {
            message: format!("Position {} exceeds maximum of {} seconds (24 hours)", position, MAX_POSITION),
        });
    }
...
pub fn validate_setting_key(key: &str) -> Result<String> {
    ...
    if !VALID_KEYS.contains(&key) {
        return Err(KiyyaError::InvalidInput {
            message: format!("Invalid setting key: '{}'. Must be one of: {:?}", key, VALID_KEYS),
        });
    }
    ...
    match key {
        "theme" => {
            if value != "dark" && value != "light" {
                return Err(KiyyaError::InvalidInput {
                    message: format!("Invalid theme value: '{}'. Must be 'dark' or 'light'", value),
                });
            }
        }
        "last_used_quality" => {
        ...
        "encrypt_downloads" | "auto_upgrade_quality" => {
            if value != "true" && value != "false" {
                return Err(KiyyaError::InvalidInput {
                    message: format!("Invalid boolean value: '{}'. Must be 'true' or 'false'", value),
                });
            }
        }
        "cache_ttl_minutes" => {
            let minutes: u32 = value.parse().map_err(|_| KiyyaError::InvalidInput {
                message: format!("Invalid cache_ttl_minutes value: '{}'. Must be a positive integer", value),
            })?;
            if minutes == 0 || minutes > 1440 {
                return Err(KiyyaError::InvalidInput {
                    message: format!("cache_ttl_minutes must be between 1 and 1440 (24 hours), got {}", minutes),
                });
            }
        }
        "max_cache_items" => {
            let items: u32 = value.parse().map_err(|_| KiyyaError::InvalidInput {
                message: format!("Invalid max_cache_items value: '{}'. Must be a positive integer", value),
            })?;
...
mod tests {
    ...
    fn test_validate_download_url() {
        ...
        assert!(validate_download_url(&format!("https://example.com/{}", "a".repeat(2048))).is_err());
    }
