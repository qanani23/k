...
use crate::models::{DiagnosticsData, CacheStats};
...
async fn add_system_info(
    ...
    info.push_str(&format!("Total Swap: {} MB\n", system.total_swap() / 1024 / 1024));
...
async fn add_database_metadata(
    ...
    metadata.push_str(&format!("Playlist Count: {}\n", memory_stats.playlist_count));
    metadata.push_str(&format!("Favorites Count: {}\n", memory_stats.favorites_count));
    metadata.push_str(&format!("Offline Content Count: {}\n", memory_stats.offline_content_count));
    metadata.push_str(&format!("Database File Size: {} MB\n", memory_stats.database_file_size / 1024 / 1024));
...
async fn add_error_logs(
    ...
    if let Ok(error_stats) = error_logging::get_error_stats(db).await {
        ...
        error_info.push_str(&format!("Unresolved Errors: {}\n\n", error_stats.unresolved_errors));
