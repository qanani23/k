// Allow unused code in models.rs - many structs are defined for future use
#![allow(dead_code)]
...
impl VersionManifest {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.latest_version.is_empty() {
            return Err(KiyyaError::validation_error(
                "latest_version",
                "Latest version cannot be empty",
            ));
        }
        if self.min_supported_version.is_empty() {
            return Err(KiyyaError::validation_error(
                "min_supported_version",
                "Minimum supported version cannot be empty",
            ));
        }
...
pub mod series {
    ...
    fn get_series_regex() -> &'static Regex {
        SERIES_REGEX
            .get_or_init(|| Regex::new(r"^(.+?)\s+S(\d{1,2})E(\d{1,2})\s*-\s*(.+)$").unwrap())
    }
    ...
    impl Version {
        ...
        pub fn parse(version_str: &str) -> Result<Self> {
            ...
            let major = parts[0].parse::<u32>().map_err(|_| {
                KiyyaError::validation_error("version", "Invalid major version number")
            })?;

            let minor = parts[1].parse::<u32>().map_err(|_| {
                KiyyaError::validation_error("version", "Invalid minor version number")
            })?;

            let patch = if parts.len() == 3 {
                parts[2].parse::<u32>().map_err(|_| {
                    KiyyaError::validation_error("version", "Invalid patch version number")
                })?
            } else {
                0
            };
...
mod tests {
    ...
    fn test_version_comparison() {
        ...
        assert_eq!(
            version::Version::compare("1.0.0", "1.0.0").unwrap(),
