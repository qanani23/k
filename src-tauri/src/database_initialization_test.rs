...
mod tests {
    ...
    async fn test_run_migrations_can_be_called_independently() {
        ...
        // With the new linear migration model, fresh databases start at version 18
        // (they skip migrations entirely since initialize() creates the full schema)
        let initial_count =
            get_migration_count(&db_path).expect("Failed to get initial migration count");
        assert_eq!(initial_count, 1, "Fresh database should have 1 migration entry (version 18)");

        // Call run_migrations() explicitly - should be a no-op since we're already at current version
        db.run_migrations().await.expect("Failed to run migrations");

        // Verify migration count is still 1 (no new migrations applied)
        let final_count =
            get_migration_count(&db_path).expect("Failed to get final migration count");

        assert_eq!(
            final_count, 1,
            "Migration count should remain 1 after calling run_migrations() on fresh database"
        );
    }
    ...
    async fn test_successful_application_startup() {
        ...
        assert!(
            progress.is_none(),
            "Should have no progress for non-existent claim"
        );
        ...
        assert!(
            setting.is_none(),
            "Should have no setting for non-existent key"
        );
    ...
    async fn test_connection_pool_initialized() {
        ...
        for _ in 0..3 {
            let _ = db
                .get_favorites()
                .await
                .expect("Connection pool should handle multiple requests");
        }
    }
