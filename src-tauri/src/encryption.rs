...
use aes_gcm::{Aes256Gcm, Nonce, KeyInit, aead::{Aead, generic_array::GenericArray}};
use rand::RngCore;
use tokio::fs::File;
...
impl EncryptionManager {
    ...
    pub async fn encrypt_file(&self, input_path: &Path, output_path: &Path) -> Result<()> {
        ...
        loop {
            ...
            let encrypted_chunk = cipher.encrypt(chunk_nonce, &buffer[..bytes_read])
                .map_err(|e| KiyyaError::Encryption {
                    message: format!("Encryption failed: {}", e),
                })?;

            // Write chunk size and encrypted data
            output_file.write_all(&(encrypted_chunk.len() as u32).to_le_bytes()).await?;
    ...
    pub async fn decrypt_file(&self, input_path: &Path, output_path: &Path) -> Result<()> {
        ...
        loop {
            ...
            let decrypted_chunk = cipher.decrypt(chunk_nonce, encrypted_chunk.as_slice())
                .map_err(|e| KiyyaError::Encryption {
    ...
    pub async fn decrypt_range(&self, input_path: &Path, start: u64, end: u64) -> Result<Vec<u8>> {
        ...
        for chunk_idx in start_chunk..=end_chunk {
            ...
            input_file.seek(SeekFrom::Start(chunk_info.file_offset)).await?;
...
mod tests {
    ...
    async fn test_decrypt_range_without_encryption_enabled() {
        ...
        write(&encrypted_path, b"dummy encrypted content").await.unwrap();
    ...
    async fn test_concurrent_range_decryption() {
        ...
        // Enable encryption and encrypt file
        let mut manager = EncryptionManager::new().unwrap();
        manager.enable_encryption("test_passphrase_concurrent").unwrap();
        manager.encrypt_file(&input_path, &encrypted_path).await.unwrap();
    ...
    async fn test_keystore_operations() {
        ...
        manager.enable_encryption("test_keystore_passphrase").unwrap();
        assert!(manager.is_encryption_enabled());
    ...
    async fn test_round_trip_single_byte() {
        ...
        manager.encrypt_file(&input_path, &encrypted_path).await.unwrap();
        manager.decrypt_file(&encrypted_path, &decrypted_path).await.unwrap();
    ...
    async fn test_round_trip_text_data() {
        ...
        write(&input_path, original_content.as_bytes()).await.unwrap();
    ...
    async fn test_round_trip_different_passphrases() {
        ...
        manager1.decrypt_file(&encrypted1_path, &decrypted1_path).await.unwrap();
        ...
        manager2.decrypt_file(&encrypted2_path, &decrypted2_path).await.unwrap();
    ...
    async fn test_decrypt_with_wrong_key_fails() {
        ...
        manager1.encrypt_file(&input_path, &encrypted_path).await.unwrap();
        let _ = manager1.disable_encryption();
        
        // Try to decrypt with different passphrase
        let mut manager2 = EncryptionManager::new().unwrap();
        manager2.enable_encryption("wrong_passphrase").unwrap();
        let result = manager2.decrypt_file(&encrypted_path, &decrypted_path).await;
    ...
    async fn test_round_trip_preserves_binary_data() {
        ...
        for (i, (&original, &decrypted)) in original_content.iter().zip(decrypted_content.iter()).enumerate() {
    ...
    async fn test_encrypted_file_is_different_from_original() {
        ...
        let mut manager = EncryptionManager::new().unwrap();
        manager.enable_encryption("test_encryption_changes_data").unwrap();
    ...
    async fn test_multiple_sequential_round_trips() {
        ...
        for i in 0..5 {
            ...
            let decrypted_content = tokio::fs::read(&decrypted_path).await.unwrap();
            assert_eq!(decrypted_content, original_content, "Round trip {} failed", i);
        }
