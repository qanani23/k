// Allow unused code in error_logging.rs - error logging features are integrated but not all functions are currently used
#![allow(dead_code)]
...
pub async fn log_error(db: &Database, error: &KiyyaError, context: ErrorContext) -> Result<()> {
...
pub async fn get_recent_errors(
    ...
    let rows = db
        .query_sql(query, vec![Value::Integer(limit as i64)])
        .await?;
...
pub async fn get_error_stats(db: &Database) -> Result<ErrorStats> {
    ...
    let total_errors: u64 = db
        .query_one_sql::<i64>("SELECT COUNT(*) FROM error_logs", vec![])
        .await?
        .unwrap_or(0) as u64;

    // Get unresolved error count
    let unresolved_errors: u64 = db
        .query_one_sql::<i64>("SELECT COUNT(*) FROM error_logs WHERE resolved = 0", vec![])
        .await?
        .unwrap_or(0) as u64;
...
pub async fn log_result_error_simple<T>(result: Result<T>, db: &Database) -> Result<T> {
    if let Err(ref error) = result {
        // Log the error but don't fail if logging fails
...
mod tests {
    ...
    async fn test_error_codes() {
        ...
        assert_eq!(
            get_error_code(&KiyyaError::gateway_error("test")),
            Some("E_GATEWAY_002".to_string())
        );
        assert_eq!(
            get_error_code(&KiyyaError::download_error("test")),
            Some("E_DOWNLOAD_001".to_string())
        );
        assert_eq!(
            get_error_code(&KiyyaError::encryption_error("test")),
            Some("E_SECURITY_001".to_string())
        );
    }
}
