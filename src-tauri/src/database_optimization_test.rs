...
mod database_optimization_tests {
    ...
    fn create_test_content_item(claim_id: &str, tags: Vec<String>, release_time: i64) -> ContentItem {
    ...
    async fn test_indices_exist() {
        ...
        assert!(indices.contains(&"idx_localcache_cleanup".to_string()), 
                "Composite index for cleanup query should exist");
        assert!(indices.contains(&"idx_localcache_tags_release".to_string()), 
                "Composite index for tag filtering with time ordering should exist");
        assert!(indices.contains(&"idx_localcache_ttl_tags".to_string()), 
                "Composite index for TTL-based queries with tag filtering should exist");
        assert!(indices.contains(&"idx_offline_meta_encrypted".to_string()), 
                "Index on encrypted column should exist");
        
        // Verify basic single-column indices
        assert!(indices.contains(&"idx_localcache_titleLower".to_string()), 
                "Index on titleLower should exist");
        assert!(indices.contains(&"idx_localcache_tags".to_string()), 
                "Index on tags should exist");
        assert!(indices.contains(&"idx_localcache_releaseTime".to_string()), 
    ...
    async fn test_query_uses_index() {
        ...
        assert!(plan_text.to_lowercase().contains("index") || plan_text.to_lowercase().contains("using"),
                "Query should use an index. Plan: {:?}", plan);
    }
    ...
    async fn test_cleanup_query_performance() {
        ...
        let cleanup_query = "SELECT claimId FROM local_cache ORDER BY lastAccessed ASC, accessCount ASC LIMIT 10";
        let plan = db.analyze_query(cleanup_query).await.unwrap();
        
        // The query plan should use the composite index for cleanup
        let plan_text = plan.join(" ");
        assert!(plan_text.to_lowercase().contains("index") || plan_text.contains("idx_localcache_cleanup"),
                "Cleanup query should use composite index. Plan: {:?}", plan);
    }
    ...
    async fn test_tag_filtering_with_composite_index() {
        ...
        let now = Utc::now().timestamp();
        let items = vec![
            create_test_content_item("movie1", vec!["movie".to_string(), "action".to_string()], now - 1000),
            create_test_content_item("movie2", vec!["movie".to_string(), "comedy".to_string()], now - 2000),
        ...
        assert_eq!(results[0].claim_id, "movie1", "First result should be most recent");
        assert_eq!(results[1].claim_id, "movie2", "Second result should be second most recent");
        assert_eq!(results[2].claim_id, "movie3", "Third result should be oldest");
    }
    ...
    async fn test_encrypted_filter_performance() {
        ...
        let plan_text = plan.join(" ");
        assert!(plan_text.to_lowercase().contains("index") || plan_text.contains("encrypted"),
                "Encrypted filter query should use index. Plan: {:?}", plan);
    }
}
