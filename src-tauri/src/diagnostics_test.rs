...
async fn test_debug_package_creation() {
    ...
    assert!(result.is_ok(), "Debug package collection failed: {:?}", result.err());
    
    let package_path = result.unwrap();
    
    // Verify the zip file was created
    assert!(package_path.exists(), "Debug package file was not created");
    assert!(package_path.extension().and_then(|s| s.to_str()) == Some("zip"), 
    ...
    assert!(file_name.starts_with("kiyya_debug_"), "Invalid debug package file name");
    assert!(file_name.ends_with(".zip"), "Invalid debug package file extension");
...
async fn test_debug_package_contains_system_info() {
    ...
    assert!(system_info.is_ok(), "system_info.txt not found in debug package");
    ...
    assert!(content.contains("Kiyya Debug Package"), "Missing header in system_info.txt");
    assert!(content.contains("System Information"), "Missing system information section");
    assert!(content.contains("OS:"), "Missing OS information");
    assert!(content.contains("Total Memory:"), "Missing memory information");
}
...
async fn test_debug_package_contains_database_metadata() {
    ...
    db.set_setting("theme", "dark").await.expect("Failed to set theme");
    db.set_setting("last_used_quality", "720p").await.expect("Failed to set quality");
    ...
    assert!(db_metadata.is_ok(), "database_metadata.txt not found in debug package");
    ...
    assert!(content.contains("Database Metadata"), "Missing database metadata section");
    assert!(content.contains("Database Version:"), "Missing database version");
    assert!(content.contains("Cache Statistics"), "Missing cache statistics");
}
...
async fn test_debug_package_sanitizes_sensitive_data() {
    ...
    db.set_setting("encryption_key", "super_secret_key_12345").await
    ...
    for i in 0..archive.len() {
        let mut file = archive.by_index(i).expect("Failed to read file from archive");
        let mut content = String::new();
        std::io::Read::read_to_string(&mut file, &mut content)
            .expect("Failed to read file content");
        
        // Verify encryption key is NOT in any file
        assert!(!content.contains("super_secret_key_12345"), 
                "Sensitive data (encryption key) found in debug package file: {}", file.name());
        assert!(!content.contains("encryption_key"), 
                "Encryption key setting found in debug package file: {}", file.name());
    }
}
...
fn test_debug_package_components_documented() {
    ...
    assert_eq!(required_components.len(), 6, 
