...
pub enum KiyyaError {
    ...
    DownloadInterrupted { bytes_downloaded: u64, total_bytes: u64 },
...
impl KiyyaError {
    ...
    pub fn download_error(message: impl Into<String>) -> Self {
        Self::Download { message: message.into() }
    }
    ...
    pub fn content_parsing_error(message: impl Into<String>) -> Self {
        Self::ContentParsing { message: message.into() }
    }
    ...
    pub fn category(&self) -> &'static str {
        match self {
            ...
            Self::Download { .. } | 
            Self::DownloadInterrupted { .. } => "download",
            ...
            Self::Search { .. } | 
            Self::QueryTimeout { .. } | 
            Self::InvalidSearchQuery { .. } => "search",
            
            Self::Playlist { .. } | 
            Self::SeriesParsing { .. } | 
            Self::EpisodeOrdering { .. } => "series",
    ...
    pub fn user_message(&self) -> String {
        match self {
            ...
            Self::AllGatewaysFailed { .. } => "All servers are currently unavailable. Please try again later.".to_string(),
            ...
            Self::DownloadInterrupted { .. } => "Download was interrupted. You can resume it later.".to_string(),
            Self::DecryptionFailed { .. } => "Failed to decrypt content. Your encryption key may be invalid.".to_string(),
            Self::ForcedUpdateRequired { .. } => "A required update is available. Please update the application to continue.".to_string(),
            Self::CodecNotSupported { .. } => "This video format is not supported on your device.".to_string(),
...
mod tests {
    ...
    fn test_error_recoverability() {
        ...
        assert!(KiyyaError::DownloadInterrupted { bytes_downloaded: 100, total_bytes: 200 }.is_recoverable());
    }
    
    #[test]
    fn test_warning_level() {
        assert!(KiyyaError::CacheTtlExpired { item_id: "test".to_string() }.is_warning_level());
        assert!(!KiyyaError::encryption_error("test").is_warning_level());
    }
    
    #[test]
    fn test_user_messages() {
        let error = KiyyaError::InsufficientDiskSpace { required: 1024 * 1024 * 100, available: 1024 * 1024 * 50 };
