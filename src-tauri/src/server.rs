...
impl LocalServer {
    ...
    pub async fn start(&mut self) -> Result<u16> {
        ...
        let movies_route = warp::path!("movies" / String)
            ...
            .and_then(move |uuid: String, range: Option<String>| {
                ...
                async move {
                    serve_content(uuid, range, active_streams, encryption_manager).await
                }
            });
        ...
        let server_handle = tokio::spawn(async move {
            server.serve_incoming(tokio_stream::wrappers::TcpListenerStream::new(listener)).await;
        });
    ...
    pub async fn register_content(&self, uuid: &str, file_path: PathBuf, encrypted: bool) -> Result<()> {
        ...
        info!("Registered content for streaming: {} (encrypted: {})", uuid, encrypted);
        Ok(())
    }
...
async fn serve_content(
    ...
    let content = if stream_info.encrypted {
        ...
        match encryption_manager.decrypt_range(&stream_info.file_path, start, end).await {
            Ok(data) => data,
            Err(e) => {
    ...
    if is_partial {
        response = response
            ...
            .header("Content-Range", format!("bytes {}-{}/{}", start, end, stream_info.file_size));
    } else {
...
mod tests {
    ...
    async fn test_local_server_register_content() {
        ...
        let result = server.register_content("test-uuid", file_path.clone(), false).await;
        assert!(result.is_ok());
    ...
    async fn test_local_server_unregister_content() {
        ...
        // Register content
        server.register_content("test-uuid", file_path, false).await.unwrap();
    ...
    async fn test_local_server_get_status() {
        ...
        server.register_content("uuid1", file_path.clone(), false).await.unwrap();
        server.register_content("uuid2", file_path, false).await.unwrap();
    ...
    async fn test_concurrent_streaming_connections() {
        ...
        // Register content
        server.register_content("test-video", file_path.clone(), false).await.unwrap();
    ...
    async fn test_encrypted_file_streaming() {
        ...
        encryption_manager.enable_encryption("test_streaming_passphrase").unwrap();
        encryption_manager.encrypt_file(&plain_file, &encrypted_file).await.unwrap();
        ...
        // Enable encryption on the server's encryption manager
        {
            ...
            server_enc_mgr.enable_encryption("test_streaming_passphrase").unwrap();
        }
        
        let port = server.start().await.unwrap();
        
        // Register encrypted content
        server.register_content("test-encrypted-video", encrypted_file.clone(), true).await.unwrap();
        ...
        // Clean up
        server.unregister_content("test-encrypted-video").await.unwrap();
        server.stop().await.unwrap();
    ...
    async fn test_encrypted_large_file_streaming() {
        ...
        // Encrypt the file
        let mut encryption_manager = EncryptionManager::new().unwrap();
        encryption_manager.enable_encryption("test_large_streaming").unwrap();
        encryption_manager.encrypt_file(&plain_file, &encrypted_file).await.unwrap();
        ...
        {
            let mut server_enc_mgr = server.encryption_manager.lock().await;
            server_enc_mgr.enable_encryption("test_large_streaming").unwrap();
        }
        
        let port = server.start().await.unwrap();
        
        // Register encrypted content
        server.register_content("large-encrypted-video", encrypted_file.clone(), true).await.unwrap();
        ...
        // Clean up
        server.unregister_content("large-encrypted-video").await.unwrap();
        server.stop().await.unwrap();
        encryption_manager.disable_encryption().unwrap();
    }
    ...
    async fn test_mixed_encrypted_and_plain_streaming() {
        ...
        // Encrypt one file
        let mut encryption_manager = EncryptionManager::new().unwrap();
        encryption_manager.enable_encryption("test_mixed_streaming").unwrap();
        encryption_manager.encrypt_file(&encrypted_source, &encrypted_file).await.unwrap();
        ...
        {
            let mut server_enc_mgr = server.encryption_manager.lock().await;
            server_enc_mgr.enable_encryption("test_mixed_streaming").unwrap();
        }
        
        let port = server.start().await.unwrap();
        
        // Register both plain and encrypted content
        server.register_content("plain-video", plain_file.clone(), false).await.unwrap();
        server.register_content("encrypted-video", encrypted_file.clone(), true).await.unwrap();
