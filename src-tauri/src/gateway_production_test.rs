...
mod tests {
        ...
        for (label, gateway_url) in gateways {
            ...
            match result {
                Ok(response) => {
                    ...
                    println!("  ✓ {} responded with HTTP {} in {}ms", 
                             label, status, elapsed.as_millis());
    ...
    async fn test_production_failover_mechanism() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                panic!("Request should succeed with at least one working gateway: {}", e);
            }
        }
    }
    ...
    async fn test_production_failover_consistency() {
        ...
        assert_eq!(priority_order[0], "https://api.na-backend.odysee.com/api/v1/proxy");
    ...
    async fn test_production_gateway_performance() {
        ...
        for (label, gateway_url) in gateways {
                ...
                match result {
                    Ok(response) => {
                        if response.status().is_success() {
                            ...
                            println!("  Request {}/{}: {}ms", i, num_requests, elapsed.as_millis());
                        } else {
                            println!("  Request {}/{}: Failed with status {}", 
                                     i, num_requests, response.status());
                        }
                    }
                    Err(e) => {
            ...
            if !response_times.is_empty() {
                ...
                println!("  Success rate: {}/{}\n", response_times.len(), num_requests);
            } else {
    ...
    async fn test_production_gateway_logging() {
        ...
        for (i, health) in gateway_client.get_health_stats().iter().enumerate() {
            ...
            println!("  {} ({}): status={}, response_time={:?}ms",
                     label,
                     health.url,
                     health.status,
                     health.response_time_ms);
