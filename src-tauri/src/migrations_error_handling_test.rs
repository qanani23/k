...
use crate::migrations::{MigrationRunner, Migration, MigrationInfo};
use crate::error::{KiyyaError, ErrorContext};
...
mod tests {
    ...
    fn test_query_failure_error_context() {
        ...
        assert!(result.is_err(), "Should fail when migrations table doesn't exist");
    ...
    fn test_row_parsing_failure_error_context() {
        ...
        conn.execute("ALTER TABLE migrations RENAME TO migrations_old", []).unwrap();
        conn.execute(
            r#"CREATE TABLE migrations (
        ...
        conn.execute(
            "INSERT INTO migrations SELECT * FROM migrations_old",
            []
        ).unwrap();
    ...
    fn test_migration_execution() {
        ...
        assert!(migration_recorded, "Migration should be recorded in history");
        ...
        assert!(migration_info.applied_at > 0, "Applied timestamp should be set");
        assert!(migration_info.checksum.is_some(), "Checksum should be calculated");
    }
    ...
    fn test_multiple_migrations_execution() {
        ...
        for migration in &migrations {
            ...
            assert!(found, "Migration {} should be in history", migration.version);
        }
        
        // Verify migrations are ordered by version
        let test_migrations: Vec<_> = history.iter()
    ...
    fn test_backward_compatibility_old_schema() {
        ...
        let result = runner.get_migration_history(&conn);
        assert!(result.is_ok(), "Should handle old schema with fallback query");
        ...
        assert_eq!(history[0].description, "", "Description should be empty string for old schema");
        assert!(history[0].checksum.is_none(), "Checksum should be None for old schema");
    ...
    fn test_backward_compatibility_mixed_schema() {
        ...
        assert_eq!(history[0].version, 1);
        assert_eq!(history[0].description, "", "Old migration should have empty description");
        assert!(history[0].checksum.is_none(), "Old migration should have no checksum");
...
mod property_tests {
    ...
    fn create_rusqlite_error(error_type: u8) -> rusqlite::Error {
        match error_type {
            ...
            1 => rusqlite::Error::InvalidColumnType(0, "version".to_string(), rusqlite::types::Type::Text),
