...
mod tests {
    ...
    async fn setup_test_db() -> Database {
        ...
        db.store_content_items(test_items).await.expect("Failed to store test items");
    ...
    async fn test_search_content_basic() {
        ...
        let results = db.search_content("Breaking Bad", Some(10)).await.expect("Search failed");
        
        assert!(results.len() >= 2, "Should find at least 2 Breaking Bad episodes, found {}", results.len());
        assert!(results.iter().all(|item| item.title.contains("Breaking Bad")));
    }
    ...
    async fn test_search_content_episode() {
        ...
        // Search for "Pilot"
        let results = db.search_content("Pilot", Some(10)).await.expect("Search failed");
        
        assert!(results.len() >= 2, "Should find at least 2 Pilot episodes, found {}", results.len());
    }
    ...
    async fn test_search_content_description() {
        ...
        let results = db.search_content("chemistry", Some(10)).await.expect("Search failed");
        
        assert!(results.len() >= 1, "Should find at least 1 result with 'chemistry' in description");
    }
    ...
    async fn test_search_content_empty_query() {
        ...
        let results = db.search_content("", Some(10)).await.expect("Search failed");
    ...
    async fn test_search_content_no_results() {
        ...
        let results = db.search_content("nonexistent_show_xyz", Some(10)).await.expect("Search failed");
        
        assert_eq!(results.len(), 0, "Should return no results for nonexistent content");
    }
    ...
    async fn test_search_content_limit() {
        ...
        let results = db.search_content("Pilot", Some(1)).await.expect("Search failed");
