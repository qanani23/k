...
use crate::models::{DownloadRequest, DownloadProgress, OfflineMetadata};
use crate::encryption::EncryptionManager;
use crate::path_security;
use reqwest::Client;
use std::path::{Path, PathBuf};
use tokio::fs::{File, create_dir_all, rename, remove_file};
...
impl DownloadManager {
    ...
    pub async fn check_disk_space(&self, required_bytes: u64) -> Result<bool> {
        ...
        for disk in system.disks() {
            if self.vault_path.starts_with(disk.mount_point()) {
                ...
                info!("Disk space check: required {} bytes, available {} bytes", 
        ...
        warn!("Could not determine disk space for vault path: {:?}", self.vault_path);
        Ok(true)
    }
    ...
    pub async fn download_content(
        ...
        info!("Starting download: {} ({})", request.claim_id, request.quality);
        ...
        if lock_path.exists() {
            warn!("Download already in progress for {} ({})", request.claim_id, request.quality);
            return Err(KiyyaError::Download {
                message: format!("Download already in progress for {} ({})", request.claim_id, request.quality),
            });
        }
        
        // Create lock file - wrap in error handling
        if let Err(e) = File::create(&lock_path).await {
            error!("Failed to create lock file for {} ({}): {}", request.claim_id, request.quality, e);
            return Err(KiyyaError::Download {
        ...
        if temp_path.exists() {
            ...
            let etag_path = self.vault_path.join(format!("{}-{}.etag", request.claim_id, request.quality));
            if etag_path.exists() {
            ...
            info!("Resuming download from byte {} (ETag: {:?})", resume_from, stored_etag);
        }

        // Get content length and check disk space
        let (content_length, server_etag, supports_range) = match self.get_content_metadata(&request.url).await {
            Ok(metadata) => metadata,
            Err(e) => {
                error!("Failed to get content metadata for {} ({}): {}", request.claim_id, request.quality, e);
        ...
        if resume_from > 0 {
            if let (Some(stored), Some(server)) = (&stored_etag, &server_etag) {
                if stored != server {
                    ...
                    let _ = remove_file(self.vault_path.join(format!("{}-{}.etag", request.claim_id, request.quality))).await;
                    resume_from = 0;
                }
            }
        }
        ...
        if let Some(total_size) = content_length {
            ...
            if let Err(e) = self.check_disk_space(remaining_bytes).await {
                error!("Insufficient disk space for {} ({}): {}", request.claim_id, request.quality, e);
                // Clean up lock file before returning
                let _ = remove_file(&lock_path).await;
                return Err(e);
            }
        ...
        let mut response = if resume_from > 0 && supports_range {
            info!("Requesting resume from byte {} with Range header", resume_from);
            match self.client
                .get(&request.url)
                ...
                Ok(resp) => resp,
                Err(e) => {
                    error!("Failed to start download for {} ({}): {}", request.claim_id, request.quality, e);
                    // Clean up lock file before returning
                    let _ = remove_file(&lock_path).await;
        ...
        let mut file = if resume_from > 0 {
            match File::options().write(true).append(true).open(&temp_path).await {
                Ok(mut f) => {
                    if let Err(e) = f.seek(SeekFrom::End(0)).await {
                        error!("Failed to seek in temp file for {} ({}): {}", request.claim_id, request.quality, e);
                        let _ = remove_file(&lock_path).await;
                ...
                Err(e) => {
                    error!("Failed to open temp file for {} ({}): {}", request.claim_id, request.quality, e);
                    let _ = remove_file(&lock_path).await;
                    return Err(KiyyaError::Io(e));
                }
            }
            ...
            match File::create(&temp_path).await {
                Ok(f) => f,
                Err(e) => {
                    error!("Failed to create temp file for {} ({}): {}", request.claim_id, request.quality, e);
                    let _ = remove_file(&lock_path).await;
                    return Err(KiyyaError::Io(e));
                }
            }
            ...
            match response.chunk().await {
                Ok(Some(chunk)) => {
                    ...
                    if let Err(e) = file.write_all(&chunk).await {
                        error!("Failed to write chunk for {} ({}): {}", request.claim_id, request.quality, e);
                        // Clean up on write error
                        drop(file);
                        let _ = remove_file(&lock_path).await;
                        
                        // Emit error event
                        let _ = app_handle.emit_all("download-error", serde_json::json!({
                ...
                Err(e) => {
                    error!("Network error during download for {} ({}): {}", request.claim_id, request.quality, e);
        ...
        if let Err(e) = file.flush().await {
            error!("Failed to flush file for {} ({}): {}", request.claim_id, request.quality, e);
            drop(file);
            let _ = remove_file(&lock_path).await;
            let _ = remove_file(&temp_path).await;
            return Err(KiyyaError::Io(e));
            ...
            match tokio::fs::metadata(&temp_path).await {
                Ok(metadata) => {
                    ...
                    if actual_size != expected_size {
                        error!("File size mismatch for {} ({}): expected {} bytes, got {} bytes", 
                ...
                Err(e) => {
                    error!("Failed to verify file size for {} ({}): {}", request.claim_id, request.quality, e);
                    let _ = remove_file(&lock_path).await;
                    let _ = remove_file(&temp_path).await;
                    return Err(KiyyaError::Io(e));
                }
        ...
        if encrypt {
            ...
            match self.encryption_manager.encrypt_file(&temp_path, &final_path).await {
                Ok(_) => {
                ...
                Err(e) => {
                    error!("Encryption failed for {} ({}): {}", request.claim_id, request.quality, e);
            ...
            if let Err(e) = rename(&temp_path, &final_path).await {
                error!("Failed to rename temp file for {} ({}): {}", request.claim_id, request.quality, e);
                let _ = remove_file(&temp_path).await;
                let _ = remove_file(&lock_path).await;
                return Err(KiyyaError::Io(e));
            }
        ...
        if let Err(e) = remove_file(&lock_path).await {
            warn!("Failed to remove lock file for {} ({}): {}", request.claim_id, request.quality, e);
        }

        let final_size = match tokio::fs::metadata(&final_path).await {
            Ok(metadata) => metadata.len(),
            Err(e) => {
                error!("Failed to get final file size for {} ({}): {}", request.claim_id, request.quality, e);
        ...
        self.total_downloads.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        self.total_bytes.fetch_add(bytes_downloaded, std::sync::atomic::Ordering::Relaxed);
        ...
        info!("Download completed: {} ({}) - {} bytes in {:.2}s (avg: {:.2} MB/s)", 
              request.claim_id, request.quality, final_size, total_elapsed,
    ...
    async fn get_content_length(&self, url: &str) -> Result<Option<u64>> {
        let (content_length, _, _) = self.get_content_metadata(url).await?;
        Ok(content_length)
    }

    pub async fn delete_content(&self, claim_id: &str, quality: &str, filename: &str) -> Result<()> {
        ...
        let temp_path = self.vault_path.join(format!("{}-{}.tmp", claim_id, quality));
        let lock_path = self.vault_path.join(format!("{}-{}.lock", claim_id, quality));
        let etag_path = self.vault_path.join(format!("{}-{}.etag", claim_id, quality));
    ...
    pub fn get_download_stats(&self) -> crate::models::DownloadStats {
        let total_downloads = self.total_downloads.load(std::sync::atomic::Ordering::Relaxed);
        let total_bytes = self.total_bytes.load(std::sync::atomic::Ordering::Relaxed);
        let total_duration_ms = self.total_duration_ms.load(std::sync::atomic::Ordering::Relaxed);
...
mod tests {
    ...
    async fn test_check_disk_space_insufficient() {
        ...
        let result = manager.check_disk_space(1024 * 1024 * 1024 * 1024 * 1024).await;
        assert!(result.is_err());
        
        if let Err(KiyyaError::InsufficientDiskSpace { required, available }) = result {
    ...
    async fn test_delete_content() {
        ...
        // Delete the content
        let result = manager.delete_content("test-claim", "720p", test_filename).await;
        assert!(result.is_ok());
    ...
    async fn test_cleanup_stale_locks() {
        ...
        let _ = filetime::set_file_mtime(&old_lock, filetime::FileTime::from_system_time(two_hours_ago));
