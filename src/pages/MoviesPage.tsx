...
export default function MoviesPage() {
  ...
  const { content: movies, loading, error, loadMore, hasMore, refetch } = useMovies(filterTag || undefined);
  ...
  const currentFilter = filterTag 
    ? CATEGORIES.movies.filters.find(f => f.tag === filterTag)
    : null;

  // Get breadcrumb
  const getBreadcrumb = () => {
    if (currentFilter) {
      return `Movies / ${currentFilter.label}`;
    }
    return 'Movies';
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-text-primary">
            {currentFilter ? `${currentFilter.label} Movies` : 'Movies'}
          </h1>
      ...
      {showFilters && (
        <div className="glass rounded-xl p-6 mb-8">
          <h3 className="text-lg font-medium text-text-primary mb-4">Filter by Genre</h3>
          <div className="flex flex-wrap gap-2">
            ...
            {CATEGORIES.movies.filters.map((filter) => (
              <button
                key={filter.tag}
      ...
      {isOfflineError ? (
        <OfflineEmptyState 
          ...
          message="You are currently offline. Connect to the internet to browse movies, or view your downloaded content."
        />
      ) : error ? (
        <div 
      ...
      {movies.length > 0 ? (
          ...
          <div className={viewMode === 'grid' ? 'content-grid' : 'space-y-4'}>
            {movies.map((movie) => (
              <MovieCard
                ...
                isFavorite={favorites.includes(movie.claim_id)}
        ...
        >
          ...
          <p className="text-text-secondary mb-6">
            {currentFilter 
              ? `No ${currentFilter.label.toLowerCase()} movies are currently available.`
